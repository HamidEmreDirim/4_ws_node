...
# Ayrıca motor dönüş yönünü sürücüde bulunan şu parametre ile ayarlanabilir: 
reverse_motors =  [0, 4]
...
class MotorManager:
    ...
    def set_position_smooth_filter(self, value_ms, slave_index=0):
       
        try:
            self.slaves[slave_index].sdo_write(0x2021, 0,
                                               bytes(ctypes.c_int16(value_ms)))
            print(f"Succes smooth parameter has been set for slave {slave_index}: {value_ms}")
        except Exception as e:
            print(f"Error writing P0.332 for slave {slave_index}: {e}")

    def set_position_fir_filter(self, value, slave_index=0):
        
        try:
            self.slaves[slave_index].sdo_write(0x2022, 0,
                                               bytes(ctypes.c_int16(value)))
            print(f"Succes FIR parameter has been set for slave {slave_index}: {value}")

        except Exception as e:
            print(f"Error writing P0.342 for slave {slave_index}: {e}")
